...
셋째, 결과를 모르면 맹목적으로 재시도하지 않는다. 타임아웃은 실패가 아니라 성공인지 실패인지 알 수 없는 상태다. 여기서 그냥 다시 보내면 이중 승인 위험이 생긴다. 카카오페이 글의 문제의식을 반영해 결과 불명을 성공과 실패와 구분되는 별도 상태로 관리한다. 승인 응답이 없으면 그 자리에서 토스 결제 조회 API로 한 번 묻는다. 조회가 DONE이면 APPROVED로 옮겨 주문을 확정하고, ABORTED와 EXPIRED는 FAILED로, CANCELED는 CANCELED로 닫는다. 조회로도 결과를 모르면 실패로 단정하지 않고 REQUESTED를 유지한 채 확인 중이라는 상태로 응답한다. 결제사 API가 미확정을 에러가 아니라 상태로 돌려주는 것과 같은 방식이다. 클라이언트는 재결제 대신 상태 조회 API를 5초 간격으로 1분까지 물어보고, 서버에서는 30초 주기 스캔이 생성 60초가 지난 REQUESTED를 같은 조회로 다시 판정한다. 부분 취소나 응답 식별자가 어긋난 건은 전이 없이 남겨 다음 주기나 사람에게 넘긴다. 전이는 모두 REQUESTED인지 확인하는 조건부 UPDATE라 스캔과 사용자 재요청이 겹쳐도 한쪽만 이긴다. 여기서도 승인 API를 다시 부르지는 않는다. 재시도가 아니라 대사다. 카카오페이는 멱등키를 붙여 재요청까지 가는데, 이 프로젝트는 문제의식을 공유하면서도 조회 후 판정에서 멈춘다.
