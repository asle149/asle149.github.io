...
수공예 작품을 한정 수량·선착순으로 파는 플랫폼(수수깡)을 만들었다. 이미 만들어 둔 작품을 파는 구조라, 오픈 순간 같은 상품에 주문이 몰려도 재고보다 많이 팔리면(oversell) 안 된다는 것이 핵심 요구였고, 재고 차감 방식은 세 가지를 전부 구현해 실측으로 골랐다. 그 기록이다.
...
**2) 조건부 UPDATE: 검사와 차감을 쿼리 한 방에 처리**
갱신 행 수 1이면 성공, 0이면 품절.
...
**3) 낙관적 락: 잠그지 않고, 커밋 때 충돌을 감지해 재시도** 
`@Version` 컬럼 하나로 감지 장치가 달린다. 재시도 루프는 트랜잭션 밖·별도 빈(facade)에 둬야 한다. 실패한 트랜잭션 안에서 다시 읽어봐야 옛 값이고, 같은 클래스 안 자기 호출은 프록시를 타지 않아 새 트랜잭션이 열리지 않기 때문이다.
...
## 같은 패턴의 재사용: 예약 만료 복구
이후 주문을 "즉시 확정"에서 "예약 후 결제"로 바꿨다. 주문 즉시 재고를 차감해 `RESERVED`로 선점하고, TTL 안에 결제 확정(confirm)이 없으면 스케줄러가 취소하며 재고를 복구한다. 여기서 조회 후 수정 레이스가 재등장한다. 스케줄러가 만료 주문을 조회한 뒤 취소하기 직전에 사용자가 confirm을 누르면?

답은 재고 차감과 같았다. 상태 전이 자체를 조건부 UPDATE로 만들었다.

```sql
-- confirm: 만료·타인 주문·중복 확정은 WHERE에서 0행으로 거부된다
UPDATE orders SET status = 'COMPLETED'
 WHERE id = ? AND buyer_id = ? AND status = 'RESERVED' AND expires_at > now();

-- 만료 취소: confirm이 먼저 커밋됐으면 0행 → 재고를 복구하지 않는다
UPDATE orders SET status = 'CANCELED'
 WHERE id = ? AND status = 'RESERVED';
```

`status = 'RESERVED'` 가드 덕에 결제 확정과 만료 취소가 경합해도 **단일 승자**만 남는다. 취소 전이가 1행 성공일 때만 재고를 +1 하고, 복구(+1)와 신규 차감(−1)이 같은 재고 행에서 겹쳐도 같은 행 UPDATE끼리는 행 락으로 직렬화되고 대기가 풀리면 최신 커밋 값으로 WHERE를 재평가하므로 순서와 무관하게 안전하다.