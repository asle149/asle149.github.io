...
만료 스캔은 1분 주기 `@Scheduled`인데, 주기를 정하며 기준이 명확해졌다. **만료 판정의 정확성은 전이 쿼리의 WHERE가 지키므로, 스캔 주기는 복구가 얼마나 늦어도 되는가에만 영향을 주는 청소 담당이다.** 정확성을 락이 아니라 쿼리 조건에 두면 그 주변 장치들은 느슨해도 된다는 것이 락 3종 비교부터 여기까지 이어진 결론이다.
